<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
        <el-form :inline="true"  class="demo-form-inline">
            <el-form-item label="标题">
                <el-input placeholder="标题" v-model="listQuery.tagName" clearable></el-input>
            </el-form-item>
            <el-form-item  label="标签类型" prop="title">
              <el-select clearable class="filter-item"  v-model="listQuery.tagType" placeholder="类型">
                <el-option label="请选择业务标签" :value="null"  ></el-option>
                <el-option label="业务标签" value="1" ></el-option>
    ...
    <el-table  :data="tableData" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column width="170px" align="center" label="标签类型">
        <template slot-scope="scope">
          <span>{{scope.row.tagType === "1"? "业务标签" : "自定义标签"}}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="addWrod" > 
      <el-form label-position="left" label-width="70px" style='width: 400px; margin-left:50px;'>
        <el-form-item class="filter-item" label="名称" prop="title">
          <el-input placeholder="名称" v-model="addUser.tagName"></el-input>
        </el-form-item>
        <el-form-item  label="标签类型" prop="title">
          <el-select clearable class="filter-item" style="width: 130px" v-model="addUser.tagType" placeholder="类型">
            <el-option label="请选择业务标签" :value="null"  ></el-option>
            <el-option label="业务标签" value="1" ></el-option>
            <el-option label="自定义标签" value="2" ></el-option>
          </el-select>
      ...
      <div slot="footer" class="dialog-footer" >
        ...
        <el-button v-else type="primary" @click="createData">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    async getList() { // 用户列表数据 1 正常查询 2 搜索查询
      ...
      const res = await this.$post('/tag/page', this.listQuery.num === 1 ? params : searchParams)
    ...
    async createData() { // 添加关键字
      if (this.addUser.tagName === null || this.addUser.tagName.trim() === '') {
        return this.warning('请输入名字')
      }
      if (this.addUser.tagName.length > 10) {
      ...
      if (!this.addUser.tagType) {
        return this.warning('请选择标签类型')
      }
    ...
    async force(item) {
      ...
      const res = await this.$post('/tag/update', params)
      if (res.returnCode === '1000') {
        this.warning('成功', 'success')
        this.getList()
      }
    },
    async batchDelete(id) { // 删除标签
      if (typeof id !== 'string') {
      ...
      this.$confirm('确定要删除吗？', '提示', {
        ...
        const res = await this.$post('/tag/remove', params)
        if (res.returnCode === '1000') {
